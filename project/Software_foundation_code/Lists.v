...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  |[] , _           => false
  | _ , []          => false
  | n1::t1 , n2::t2 => if eqb n1 n2 then eqblist t1 t2
                                    else false
  end.
...
Proof.
  intros f H n1 n2 H1. 
  assert (H2: n1 = f (f n1)) by apply H.
  assert (H3: n2 = f (f n2)) by apply H.
...
Proof.
  intros.
  assert (H1: rev (rev l1) = l1) by apply rev_involutive.
  assert (H2: rev (rev l2) = l2) by apply rev_involutive.
  rewrite <- H1. rewrite H.
  exact H2.
Qed.
...
Proof.
  intros. simpl. rewrite eqb_id_refl.
  reflexivity.
Qed.
(** [] *)

...
Proof.
  ...
  assert (H: p + n = (n + p)).
    { rewrite add_comm. reflexivity. }
  rewrite add_comm. rewrite <- H. rewrite add_assoc. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite mult_n_1. rewrite mul_0_r.
...
Proof.
  ...
  - rewrite <- mult_n_Sm.  rewrite IHp'.
    ...
    rewrite add_shuffle3. rewrite add_assoc.
    rewrite add_shuffle3. rewrite add_assoc.
    reflexivity.
Qed.
...
Proof.
  intros. induction p as [|p' IHp'].
  - rewrite mul_0_r. rewrite mul_0_r.
    rewrite mul_0_r. reflexivity.
  - rewrite mul_n_Sm. rewrite mul_n_Sm. rewrite mul_comm.
    rewrite mult_plus_distr_r. rewrite mul_comm.
    rewrite <- IHp'.
    rewrite mul_comm with (n := m * p') (m := n).
    reflexivity.
Qed.
